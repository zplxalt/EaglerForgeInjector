# EaglerForgeInjector
EaglerForge Injector is a tool that uses regular expressions to attach itself to internal code inside of raw EaglercraftX offline downloads (sounds easy, isn't at all).why

### Documentation fragments
[Injector Architecture](architecture.md)

[TeaVM quirks](quirks.md)

[Modding tutorials](tutorials/index.md)

[Modding API documentation](apidoc/index.md)

[Compiling the client for EaglerForgeInjector](compiling_client.md)

[eaglercraftXopts and search parameters](config.md)
